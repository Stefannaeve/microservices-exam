...
public class ApiResponseBuilder<T> {
    ...
    public ApiResponse<T> success(T value, HttpStatus status) {
        return new ApiResponse.Success<T>(Optional.ofNullable(value), status);
    }

    public ApiResponse<T> success(T value) {
        return new ApiResponse.Success<T>(Optional.ofNullable(value), HttpStatus.OK);
    }
    ...
    public ApiResponse<T> failure(T value, HttpStatus status, String errorMessage) {
        return new ApiResponse.Failure<T>(Optional.ofNullable(value), status, errorMessage);
    }
